...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <header className="bg-[#003580] px-34 sm:px-34 py-0 flex flex-col gap-1">
  {/* Top Right Actions */}
  <div className="w-full flex items-center justify-between px-4 sm:px-4 py-8 sm:py-8 sm:justify-end gap-3">
  ...
  <p className="text-base sm:text-xl md:text-2xl mt-3 font-sans">Search low prices...</p>
        ...
        <div className="flex gap-4 px-11 mt-10">
          {[{ src: "/mumbai.png", title: "Mumbai" }, { src: "/chennai.png", title: "Chennai" }, { src: "/varanasi.png", title: "Varanasi" }].map((img, i) => (
            <div key={i} className="relative w-full sm:w-[380px] h-[220px] rounded-lg shadow-md overflow-hidden">
              <img src={img.src} alt={img.title} className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/30">
                <h2 className="absolute top-4 left-4 text-white text-2xl font-bold">{img.title}</h2>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="px-11 mt-10">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Explore destinations in India</h2>
              ...
              {carouselItems.map((item, index) => (
                <CarouselItem key={index} className="basis-full sm:basis-[48%] md:basis-[18%] px-2">
                  <div className="flex flex-col items-center">
                    ...
                    />
                    <h2 className="mt-2 text-sm font-semibold text-gray-800 truncate text-center">{item.name}</h2>
                    <p className="text-xs text-gray-500 truncate text-center">{item.location}</p>
                  </div>
                </CarouselItem>
              ))}
            </CarouselContent>
                ...
                <CarouselItem key={index} className="basis-full sm:basis-[48%] md:basis-[23%] px-2 py-4">
                  <div className="flex flex-col w-full">
                    ...
                    <div className="flex flex-col gap-1 mt-2">
                      ...
                      <h2 className="text-sm font-semibold text-gray-900 leading-snug line-clamp-2">{item.name}</h2>
                      <p className="text-xs text-gray-600">{item.location}</p>
                      <div className="flex items-center gap-1 text-xs mt-1">
                        <span className="bg-blue-600 text-white px-1.5 py-0.5 rounded text-xs font-semibold">{item.rating}</span>
                        <span className="text-gray-800 font-medium">{item.ratingText}</span>
                        <span className="text-gray-500">({item.reviews} reviews)</span>
                      </div>
                      <span className="text-xs text-green-700 font-medium mt-1">Getaway Deal</span>
                      <div className="mt-1 text-xs text-gray-800">
                        2 nights <span className="line-through text-gray-500 ml-1">{item.originalPrice}</span>{" "}
                        <span className="font-bold text-red-600 ml-1">{item.discountedPrice}</span>
                      </div>
                    </div>
                  </div>
                </CarouselItem>
